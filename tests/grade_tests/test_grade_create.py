...
class TestGrade:
    def test_grade_create(self, university_api_session_admin, tmp_teacher_id, tmp_student_id):
        university_service = UniversityService(university_api_session_admin)
        log.step(1, "Create grade")
        grade_response = university_service.create_grade(GradePostRequest(teacher_id=tmp_teacher_id,
                                                                          student_id=tmp_student_id,
                                                                          grade=randint(Grade.MIN, Grade.MAX)))
        log.step(2, "Verification for grade ids (teacher_id, student_id)")
        assert grade_response.teacher_id == tmp_teacher_id and grade_response.student_id == tmp_student_id, (
            f"Expected id for teacher or student: teacher_id - {tmp_teacher_id}, student_id - {tmp_student_id}\n"
